...
class DATA(object):
    def gen_feature(self, imgpath):
        ...
        img = img / 255.0
        norm = img * img
        img = img / norm.sum() #norm is not so important although in theory it is key
        return img
...
#        img = np.abs(img)
        img = (img - img.min()) * 255.0 / (img.max() - img.min())
        img = np.uint8(img)
        img = cv2.resize(img, (PARAM().imgh * 5,PARAM().imgw * 5) )
...
#    draw_weight(net)
    while iterator < 50000:
        ...
        if iterator % 50 == 0:
            ...
            draw_weight(net)
            draw_prd(prd)
            with open('dl.dat', 'wb') as f:
                pickle.dump((train,predict,net), f)
