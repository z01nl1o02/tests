...
//  char partitions[label]; // 't' : text, 'b' : background
 // int ntext[label]; // Number of text partitions per label
  ...
  std::vector<int> ntext, nbg;
  partitions.resize(label);
  ntext.resize(label);
  nbg.resize(label);
...
int main(void)
{
	...
	std::string input_file = "g:/2.jpg";
	std::string output_file = "g:/output.jpg";

	int winx = 7, winy = 7, optK = 1.5;
	std::string auxMethod = "n";

	cv::Mat input = cv::imread(input_file, 0);
	cv::Mat niblack(input.rows, input.cols, CV_8U);
	auxBinarization(input, niblack, winx, winy, optK, 128, auxMethod);

	for (int k = 0; k < 2; k++)
	{
		cv::dilate(niblack, niblack, cv::Mat());
		cv::erode(niblack, niblack, cv::Mat());
	}

	cv::Mat copy;
	cv::threshold(niblack, copy, 128, 255, cv::THRESH_BINARY);
	niblack = niblack * 0;
	for (int y = 1; y + 1 < copy.rows; y++)
	{
		for (int x = 1; x + 1 < copy.cols; x++)
		{
			if (copy.at<uchar>(y, x) == 0) continue;
			int sum = copy.at<uchar>(y - 1, x - 1) +
				copy.at<uchar>(y - 1, x) +
				copy.at<uchar>(y - 1, x + 1) +
				copy.at<uchar>(y + 1, x - 1) +
				copy.at<uchar>(y + 1, x) +
				copy.at<uchar>(y + 1, x + 1) +
				copy.at<uchar>(y, x - 1) +
				...
				copy.at<uchar>(y, x + 1);
			if (sum <= 2)
				niblack.at<uchar>(y, x) = 0;
			else
				niblack.at<uchar>(y, x) = 255;
		}
	}

	{
		std::vector< std::vector<cv::Point> > contours;
		cv::Mat tmp = niblack.clone();
		cv::findContours(tmp, contours, cv::RETR_CCOMP, cv::CHAIN_APPROX_SIMPLE);
		
		std::vector< std::vector<cv::Point> > contours_valid;
		for (auto contour = contours.begin(); contour != contours.end(); contour++)
		{
			if (contour->size() < 8) continue;
			if (contour->size() > 100) continue;
			cv::RotatedRect rect_rot = cv::minAreaRect(*contour);
			int len = sqrt(rect_rot.size.width * rect_rot.size.width + rect_rot.size.height * rect_rot.size.height);
			if (len < 10) continue;

			cv::Point lt = (*contour)[0], br = (*contour)[0];
			for (int k = 1; k < contour->size(); k++)
			{
				if (lt.x > (*contour)[k].x) lt = (*contour)[k];
				if (br.x < (*contour)[k].x) br = (*contour)[k];
			}
			float dx = br.x - lt.x;
			float dy = -1 * (br.y - lt.y);

			if (abs(dx) < abs(dy)) continue;

			float angle = atan2(dy, dx) * 180 / CV_PI;
			if( abs(angle) < 40 || abs(angle - 180) < 40)
				contours_valid.push_back(*contour);
		}

		niblack = niblack * 0;
		cv::cvtColor(niblack, niblack, cv::COLOR_GRAY2BGR);
		for (int k = 0; k < contours_valid.size(); k++)
		{
			cv::drawContours(niblack, contours_valid, k, CV_RGB(255, 0, 0), 1);
		}
	}
	cv::imwrite(output_file, niblack);
	return 0;
}
