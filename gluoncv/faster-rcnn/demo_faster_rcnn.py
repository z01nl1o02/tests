...
if not os.path.exists("output"):
    os.makedirs("output")
filenames = []
with open('G:/MSDataset/VOC2007/ImageSets/Main/test.txt','rb') as f:
    for name in f:
        name = name.strip()
        if name == "":
            continue
        filenames.append(os.path.join('G:/MSDataset/VOC2007/JPEGImages',name+'.bmp'))
for im_fname in filenames:
...
# results. We slice the results for the first image and feed them into `plot_bbox`:
    ...
    plt.savefig(os.path.join("output",os.path.split(im_fname)[-1])+'.jpg')
    #plt.show()
